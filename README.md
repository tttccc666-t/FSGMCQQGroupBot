...
集成了 **DeepSeek 大模型对话**、**阿里云通义万相 AI 绘图** 以及 **Qwen-VL 视觉识别** 功能。  
拥有可爱的“猫娘”人设，支持戳一戳互动、进群欢迎以及**看图说话**。  
...
- **👀 视觉识别 (新)**：接入阿里云 `qwen-vl-plus` 模型。
  - **看图说话**：发送图片并 @机器人，它能看懂图片内容并进行回复（例如：“这张图里的猫猫好可爱！”）。
- **🎨 AI 绘图**：接入阿里云 `qwen-image` 系列模型，支持高质量文生图。
...
3. **API Key**:
   ...
   - 阿里云 DashScope API Key (用于 **绘图** 和 **识图**)
